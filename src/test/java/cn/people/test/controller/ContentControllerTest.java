...
import cn.people.controller.vo.ContentRecycledListVO;
...
/*        param.setDtype(1);
    ...
    public void testGsetContentVersionPaged() throws Exception {
        ContentRecycledListVO param=new ContentRecycledListVO();
        param.setPageSize(11);
        MvcResult mvcResult = mockMvc.perform( 
            post("/content/97f8554eb2ad1f0bf480df62902e3c60/version?pageNo=0&pageSize=10").contentType(MediaType.APPLICATION_JSON)
            .content(JSON.toJSONString(param)))
            .andExpect(status().isOk())
            //.andExpect(jsonPath("$.data.success").value(true))
            .andReturn(); 
